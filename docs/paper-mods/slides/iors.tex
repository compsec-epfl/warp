...
% --------------------------------------------------------------------
\begin{frame}[fragile]{The integration trait already exists: \texttt{ark-vc}}
The single integration gap, now filled by \texttt{ark-vc} + \texttt{ark-mt}:

\begin{lstlisting}
pub trait MerkleCommitment<H: MerkleHasher, S: MerkleShape> {
    fn commit(&self, msg: &[H::Symbol]) -> Committed<...>;
    fn open(&self, td: &Committed<...>, indices: &[usize])
        -> OpeningProof<...>;
    fn check(&self, root: &H::Digest,
             opening: &Opening<H::Symbol>,
             proof: &OpeningProof<...>) -> bool;
}

// Multi-vector variant: one root over m interleaved codewords,
// one OpeningProof for arbitrary-many indices. WARP uses this.
pub struct MultiVectorMerkleCommitment<H, S, F> { ... }
\end{lstlisting}

\vspace{0.4em}
\begin{itemize}\small
  \item Single multi-opening proof per commitment (fewer auth paths than per-index).
  \item Backend-agnostic: \texttt{Blake3FieldHasher}, Poseidon hasher, hybrid stacks.
  \item WARP migrated onto it: \texttt{Path<MT>} $\to$ \texttt{OpeningProof<LayerGroup<H>>}; \texttt{MerkleTree<MT>} $\to$ \texttt{MultiVectorCommitted<H, S, F>}; \texttt{H: MerkleHasher} replaces \texttt{MT: Config} across phases. 33/33 tests still green.
\end{itemize}
\end{frame}
