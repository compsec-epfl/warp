...
\Pesat{} phase interleaves \(l_1\) codewords into a single ark-vc
commitment (see \texttt{build\_codeword\_leaves} in
\codemod{src/protocol/phases/pesat.rs}), so one commitment covers many
oracles. The Rust \codemod{Oracle} struct therefore stores only \(f\)
and the lazy \(\MLE{f}\); the \texttt{ark\_vc::Committed} state
implementing \(\Commit{f}\) is tracked by the enclosing data structure
