...
use crate::crypto::vc::{self, scheme};
    ...
    > WARP<F, P, C>
{
    ...
    /// here with `code.code_len()` leaves; swap the hasher family by
    /// editing `src/crypto/vc.rs`.
    pub fn new(config: WARPConfig<F, P>, code: C, p: P) -> WARP<F, P, C> {
        let scheme = scheme::<F>(code.code_len());
        Self {
            params: WARPParams {
                _f: PhantomData,
                config,
    ...
    > AccumulationScheme<F> for WARP<F, P, C>
{
    ...
    fn prove(
        ...
        let td = {
            ...
            let leaves = crate::crypto::merkle::per_element_leaves(tc.f.evals());
    ...
    fn decide(
        ...
        let leaves = crate::crypto::merkle::per_element_leaves(&acc_witness.f[0]);
...
// Silence unused-import warning from the `vc::` re-export — referenced
// via `crate::crypto::vc::scheme` in the impl body above.
#[allow(unused_imports)]
use vc as _vc_module_alias;
